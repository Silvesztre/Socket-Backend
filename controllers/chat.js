...
exports.createMessage = async (req, res, next) => {
  try {
    ...
    if (!isUserInChatRoom) {
      return res
        .status(401)
        .json({ message: "Unauthorized: User not in chat room" });
    }

    const newMessage = await createMessageAsync(
      chatRoomId,
      senderId,
      content,
      messageType
...
exports.addUserToChatRoom = async (req, res, next) => {
  try {
    ...
    if (!chatRoomId) {
      return next(
        new AppError("Bad request: Must provide chatRoomId and a userId.", 400)
      );
    }
    const chatRoom = await getChatRoomByChatRoomIdAsync(chatRoomId);
    if (!chatRoom) {
    ...
    if (isUserInChatRoom) {
      return res
        .status(400)
        .json({ message: "You are already in the chat room." });
    }
